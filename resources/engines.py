...
class QwenVLEngine:
    ...
    def __init__(self, model_id: str, device: str = "cuda", dtype: str = "auto",
                 ...
                 default_adapter: str | None = None) -> None:
    ...
    def load(self) -> None:
        ...
        if self.adapter and self.adapters:
            raise ValueError("use either adapter= or adapters=, not both")
        if self.adapters:
            ...
            first = names[0]
            logger.info("Applying switchable LoRA adapter %s from %s …",
                        first, self.adapters[first])
            ...
            for name in names[1:]:
                logger.info("Loading switchable LoRA adapter %s from %s …",
            ...
            selected = self.default_adapter or first
            self.set_adapter(selected)
            logger.info("Loaded %d switchable adapters; active=%s.",
                        len(self.adapters), selected)
        elif self.adapter:
            from peft import PeftModel
        ...
        logger.info("Model ready (%s%s).", "Qwen3-VL" if self._is_qwen3vl else "Auto-ITT",
                    "+LoRA" if self.adapter or self.adapters else "")

    def set_adapter(self, name: str) -> None:
        """Activate one already-loaded PEFT adapter without reloading weights."""
        if not self.adapters:
            if self.adapter and name in {"default", self.active_adapter}:
                return
            raise RuntimeError("no switchable adapters were loaded")
