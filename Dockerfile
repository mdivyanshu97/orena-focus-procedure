...
RUN python -c "import torch; flags = torch._C._cuda_getArchFlags() or ''; cuda = torch.version.cuda or ''; print('torch', torch.__version__, '| cuda', cuda, '|', flags); assert cuda.startswith('12.8'), 'torch was replaced by a CUDA ' + cuda + ' build; the L40S driver cannot run it'; assert 'sm_120' in flags and 'sm_86' in flags, 'torch build misses a required GPU architecture: ' + repr(flags)"

# ── Model definition + weights ────────────────────────────────────────────────
# resources/w64/ and resources/nw2/ must contain the two public merged
# checkpoints. Populate them with `python scripts/download_weights.py` before
# building.
#
